...
Terrain::Terrain(int resolution, float interval)
{
   ...
   for (int i = 0; i < resolution*resolution; i++)
   {
	...
	for ( int z = 1; z < m_resolution-2; z++)
	{
		if(!(z % 2))
		{
			for ( int x = 1; x < m_resolution-1; x++)
			{
				m_vertices.push_back(glm::vec3((float)x/(m_resolution-2), getHeight(x,z),(float)z/(m_resolution-2)));
				m_normals.push_back(calculateNormal(x,z));
				m_vertices.push_back(glm::vec3((float)x/(m_resolution-2), getHeight(x,z+1),(float)(z+1.0)/(m_resolution-2)));
				m_normals.push_back(calculateNormal(x,z+1));

			}
		}else
		{
			for ( int x =  m_resolution-2; x >=1; x--)
			{
				m_vertices.push_back(glm::vec3((float)x/(m_resolution-2), getHeight(x,z),(float)z/(m_resolution-2)));
				m_normals.push_back(calculateNormal(x,z));
				m_vertices.push_back(glm::vec3((float)x/(m_resolution-2), getHeight(x,z+1),(float)(z+1.0)/(m_resolution-2)));
    ...
    //generate 2 vertex buffer object (VBO)
...
Terrain::~Terrain()
{
	...
	glDeleteBuffers(1, &m_vboHandles[0]);
	glDeleteBuffers(1, &m_vboHandles[1]);
}
