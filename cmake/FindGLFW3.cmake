#
# Try to find GLFW3 library and include path.
# Once done this will define
#
# GLFW3_FOUND
# GLFW3_INCLUDE_PATH
# GLFW3_LIBRARY
# 


set(GLFW3_ROOT_ENV $ENV{GLFW3_ROOT})

IF (MINGW)
    FIND_PATH( GLFW3_INCLUDE_PATH GLFW/glfw3.h
        ${GLFW3_ROOT_ENV}/include
    )

    FIND_LIBRARY( GLFW3_LIBRARY
        NAMES glfw3
        PATHS
        ${GLFW3_ROOT_ENV}/lib-mingw
        ${GLFW3_ROOT_ENV}/lib
    )

ELSEIF (MSVC)
    FIND_PATH( GLFW3_INCLUDE_PATH GLFW/glfw3.h
        ${GLFW3_ROOT_ENV}/include
    )

    IF (MSVC10)
        FIND_LIBRARY( GLFW3_LIBRARY
            NAMES glfw3
            PATHS
            ${GLFW3_ROOT_ENV}/lib-msvc100
            ${GLFW3_ROOT_ENV}/lib
        )	
	ELSEIF (MSVC12)
		FIND_LIBRARY( GLFW3_LIBRARY
            NAMES glfw3
            PATHS
            ${GLFW3_ROOT_ENV}/lib-msvc12
            ${GLFW3_ROOT_ENV}/lib-vc2013
        )
	ELSEIF (MSVC14)
        FIND_LIBRARY( GLFW3_LIBRARY
            NAMES glfw3
            PATHS
            ${GLFW3_ROOT_ENV}/lib-vc2015
            ${GLFW3_ROOT_ENV}/lib-msvc14
            ${GLFW3_ROOT_ENV}/lib
        )
    ELSE()
        FIND_LIBRARY( GLFW3_LIBRARY
            NAMES glfw3
            PATHS
            ${GLFW3_ROOT_ENV}/lib-msvc110
            ${GLFW3_ROOT_ENV}/lib
        )
    ENDIF ()
ELSEIF(APPLE)

    FIND_PATH(GLFW3_INCLUDE_PATH GLFW/glfw3.h DOC "Path to GLFW include directory."
	HINTS ${GLFW3_ROOT_ENV}/include
	PATHS /usr/include /usr/local/include /opt/local/include
    )
    
    FIND_LIBRARY( GLFW3_LIBRARY
        NAMES libglfw3.a glfw3 glfw
        PATHS $ENV{GLFW3_ROOT_ENV}/build/src /usr/lib /usr/local/lib /opt/local/lib
    )

ELSE()

	# slightly dirty trick
	# find subfolders suspected to be multi arch folders


	MACRO(MULTI_ARCH_SUBDIRS result curdir)
	  FILE(GLOB children64 RELATIVE ${curdir} ${curdir}/*64*)
	  FILE(GLOB children86 RELATIVE ${curdir} ${curdir}/*86*)
	  set(children "${children64}" "${children86}")
	  list(REMOVE_DUPLICATES children)
	  SET(dirlist "")
	  FOREACH(child ${children})
	    IF(IS_DIRECTORY ${curdir}/${child})
	      LIST(APPEND dirlist ${curdir}/${child})
	    ENDIF()
	  ENDFOREACH()
	  SET(${result} ${dirlist})
	ENDMACRO()


	MULTI_ARCH_SUBDIRS(SUBDIRS /usr/lib)
	MULTI_ARCH_SUBDIRS(SUBDIRS_INCLUDE /usr/include)
	FIND_PATH(GLFW3_INCLUDE_PATH GLFW/glfw3.h
	PATHS ${SUBDIRS_INCLUDE})
	FIND_LIBRARY(GLFW3_LIBRARY
        NAMES glfw3 glfw
	PATH_SUFFIXES dynamic lib64
	PATHS ${SUBDIRS}) 
ENDIF ()



SET(GLFW3_FOUND "NO")
IF (GLFW3_INCLUDE_PATH AND GLFW3_LIBRARY)
	SET(GLFW3_LIBRARIES ${GLFW3_LIBRARY})
	SET(GLFW3_FOUND "YES")
    message("EXTERNAL LIBRARY 'GLFW3' FOUND")
ELSE()
    message("ERROR: EXTERNAL LIBRARY 'GLFW3' NOT FOUND")
ENDIF (GLFW3_INCLUDE_PATH AND GLFW3_LIBRARY)
